...
function htmlForKeyboardWithOctavesOLD(numberOfOctaves, startOctave, showLabels, withShiftButtons, withNoteSelection, withClefSelection) {
...
function htmlForKeyboardWithOctaves(numberOfOctaves, startOctave, showLabels, withShiftButtons, withNoteSelection, withClefSelection) {
    ...
    for (var i = 0; i < _displayedOctaves; i++) {
        if (showLabels) {
            keyhoardHTML += '\
            <li class="whiteKey" id="noteC' + (currentOctave + 1) +  '"><p>C' + (currentOctave + 1) + '</p></li>\n'
        } else {
            keyhoardHTML += '\
            <li class="whiteKey" id="noteC' + (currentOctave + 1) +  '"></li>\n'
        }

        keyhoardHTML += '\
            <li class="blackKeyCF" id="noteC#' + (currentOctave + 1) + '|Db' +(currentOctave + 1) +  '"></li>\n'

        keyhoardHTML += '\
            <li class="whiteKey" id="noteD' + (currentOctave + 1) +  '"></li>\n\
            <li class="blackKeyDA" id="noteD#' + (currentOctave + 1) + '|Eb' +(currentOctave + 1) +  '"></li>\n\
            <li class="whiteKey" id="noteE' + (currentOctave + 1) +  '"></li>\n\
            <li class="whiteKey" id="noteF' + (currentOctave + 1) +  '"></li>\n\
            <li class="blackKeyCF" id="noteF#' + (currentOctave + 1) + '|Gb' +(currentOctave + 1) +  '"></li>\n\
            <li class="whiteKey" id="noteG' + (currentOctave + 1) +  '"></li>\n\
            <li class="blackKeyG" id="noteG#' + (currentOctave + 1) + '|Ab' +(currentOctave + 1) +  '"></li>\n\
            <li class="whiteKey" id="noteA' + (currentOctave + 1) +  '"></li>\n\
            <li class="blackKeyDA" id="noteA#' + (currentOctave + 1) + '|Bb' +(currentOctave + 1) +  '"></li>\n\
            <li class="whiteKey" id="noteB' + (currentOctave + 1) +  '"></li>\n'
        currentOctave++
    }
    keyhoardHTML += '\
        </ul>\n'
    ...
    var html = '\
        <div class="DA-Keyboardcontainer">\n\
		    <div id="antesTecladoContainer"></div>'
    if (withShiftButtons) {
        html += '\
            <button type="button" id="lowerOctave" onclick="lowerOctave()">˂</button>\n'
                + keyhoardHTML + '\n\
    ...
    html += '\
	        <div id="aposTecladoContainer"></div>\n\
        </div>'
...
var codigosNotas = ["C", "C#|Db", "D", "D#|Eb", "E", "F", "F#|Gb", "G", "G#|Ab", "A", "A#|Bb", "B"];
...
function soltarTodasNotas() {
	...
	for (i = 1; i <= 7; i++) 
	{
		for (j = 0; j < 12; j++) 
		{
			notaOitava = codigosNotas[j].replace("|", "" + i + "|") + i;
			var elementoNota = document.getElementById("note" + notaOitava)
...
function pressionarNotas(notasEOitavas, cores, tipoDestaque, limparNotasJaPressionadas) {
	...
	{
		var notaEOitavaAjustada = getNotaAjustada(arrNotasEOitavas[i])
		...
		if (elementoNota != undefined) 
		{
			var elemNotaRect = elementoNota.getBoundingClientRect()

			//var elemMarcador = document.getElementById("marcadorNota");
			
			if (tipoDestaque == 'circulo') 
			{
				var elemMarcador = criarMarcadorNota(arrCores[i]);
				var elemMarcadorRect = elemMarcador.getBoundingClientRect();
				
				elemMarcador.style.left = "" + ( elemNotaRect.left + (elemNotaRect.width / 2) - (elemMarcadorRect.width / 2) ) + "px"
				elemMarcador.style.top = "" + (elemNotaRect.top + elemNotaRect.height - elemMarcadorRect.height - 8) + "px"			
			}
			else
			{
				elementoNota.style.backgroundColor = arrCores[i];
			}
		}
	}
}

function getNotaAjustada(notaEOitava) {
	var notaSemOitava = notaEOitava.substring(0, notaEOitava.length - 1)
	var oitava = notaEOitava.substring(notaEOitava.length - 1)
	
	switch (notaSemOitava) {
		case "C#":
		case "Db":
			return "C#" + oitava + "|Db" + oitava;
		case "D#":
		case "Eb":
			return "D#" + oitava + "|Eb" + oitava;
		case "F#":
		case "Gb":
			return "F#" + oitava + "|Gb" + oitava;
		case "G#":
		case "Ab":
			return "G#" + oitava + "|Ab" + oitava;
		case "A#":
		case "Bb":
			return "A#" + oitava + "|Bb" + oitava;
		default:
			return notaEOitava;
	}
}
